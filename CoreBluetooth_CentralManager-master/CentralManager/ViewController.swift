...
class ViewController: UIViewController, CBCentralManagerDelegate, UITableViewDelegate, UITableViewDataSource, CBPeripheralDelegate {
    ...
    let kServiceUUID = "8D7FCC36-C248-4750-BE34-3EAC5E7C2A55"
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        if let name = peripheral.name {
            if (!checkIfExisted(name)) {
                print("didDiscoverPeripheral \(peripheral.name ?? "did not discover")")
...
////                print("\(Date). - Awaked peripheral \(String(describing: awakedPeripheral.name))")
//                let localName = awakedPeripheral.name
//                print(localName ?? "")
////                localName == "iPad" else {
////                    return
////                }
